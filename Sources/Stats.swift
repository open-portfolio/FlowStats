...
public class Stats<T: BinaryFloatingPoint & Real>: Hashable {
    ...
    public lazy var count: T = .init(values.count)

    public lazy var summed: T = values.reduce(0, +)
    ...
    public lazy var min: T? = minAndMax?.0

    public lazy var max: T? = minAndMax?.1
...
//    }()
    ...
    private lazy var minAndMax: (T, T)? = values.minAndMax()
